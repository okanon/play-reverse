...
abstract class RestTemplateController @Inject()
  ...
  def reverseProxy(path: String) = Action.async(parse.raw) { request: Request[RawBuffer] =>
    ...
    var proxyRequest =
      ws.url("https://github.com" + request.uri)
        .withFollowRedirects(false)
        .withMethod(request.method)
        .withHttpHeaders(
          request.headers.replace(HOST -> "github.com").toSimpleMap.toSeq: _*)
    ...
    proxyRequest.stream().map { response: WSResponse =>
      ...
      val headers = response.headers
                      ...
                      .filterNot(v => v._1.equals(CONTENT_TYPE) || v._1.equals(CONTENT_LENGTH))
